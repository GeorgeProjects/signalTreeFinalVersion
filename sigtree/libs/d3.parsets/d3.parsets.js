...
(function() {
  d3.parsets = function() {
    var event = d3.dispatch("sortDimensions", "sortCategories"),
        ...
        tension0,
        duration = 500,
        mouseoverCallbackEvent = null, //Edited by Zeek
        mouseoutCallbackEvent = null; //Edited by Zeek
      ...
      selection.each(function(data, i) {
        ...
        function updateRibbons() {
          ...
          mouse.enter().append("path")
              ...
              .on("mouseover.parsets", function(d) {  //Edited by Zeek
                if (mouseoverCallbackEvent)
                  mouseoverCallbackEvent(d)
              })
              .on("mouseout.parsets", function(d) {   //Edited by Zeek
                if (mouseoutCallbackEvent)
                  mouseoutCallbackEvent(d)
              })
    ...
    //Edited By Zeek
    parsets.mouseoverCallback = function(_) {
      mouseoverCallbackEvent = _;
      return parsets;
    }

    //Edited By Zeek
    parsets.mouseoutCallback = function(_) {
      mouseoutCallbackEvent = _;
      return parsets;
    }    
    ...
    function layout(tree, dimensions, ordinal) {
      ...
      dim.categories.forEach(function(c) {
        ...
        if (tree.children.hasOwnProperty(k)) {
          recurse(c, {node: tree.children[k], path: k, id: "-" + k, index: k}, 1, ordinal(k));  //edited by Zeek
        }
      });
      ...
      function recurse(p, d, depth, major) {
        ...
        dimension.categories.forEach(function(c) {
          ...
          child.index = d.index + "-" + k;   //edited by Zeek
          child.id = d.id + "-" + k;  //edited by Zeek
          child.id = new sigtree.dataProcessor().cleanID(child.id);  //edited by Zeek
